...
class EdgarFilingsSpider(scrapy.Spider):
    ...
    def __init__(self, cik=None, form_type='10-K', year='All', limit='all', *args, **kwargs):
    ...
    def start_requests(self):
        if not self.cik:
            ...
            yield scrapy.Request('https://www.sec.gov/cgi-bin/current?q1=0&q2=0&q3=', callback=self.parse_recent)
            return
            ...
            if (self.form_type == 'ALL' or self.form_type in f_type) and \
               ('All' in self.years or f_year in self.years):
                ...
                yield scrapy.Request(
                    ...
                    meta={'date': f_date, 'type': f_type, 'title': f"{f_type}: {data.get('name')}"}
                )
