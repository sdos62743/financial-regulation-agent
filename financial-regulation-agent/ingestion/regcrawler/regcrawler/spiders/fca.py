...
class FcaSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        if not search_items:
            log_error(f"No search items found on {response.url}. Selectors may need update.")
            return
        ...
        for item in search_items:
            ...
            category = item.css('.search-item__category::text, .result-category::text').get()
        ...
        next_page = response.css('li.next a::attr(href), a.pagination__next::attr(href)').get()
        if next_page:
    ...
    def parse_fca_content(self, response):
        ...
        try:
            ...
            content_blocks = response.css('section.component--text-block, .field--name-body').getall()
            ...
            formatted_date = raw_date.strip() if raw_date else datetime.now().strftime("%Y-%m-%d")
    ...
    def _map_category_to_type(self, category):
        ...
        cat = category.lower()
        if "speech" in cat: return "speech"
        if "statement" in cat: return "statement"
        if "policy" in cat: return "policy_statement"
        if "warning" in cat: return "enforcement"
