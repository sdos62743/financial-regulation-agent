...
class FomcSpider(scrapy.Spider):
    ...
    start_urls = ["https://www.federalreserve.gov/monetarypolicy/fomc_historical_year.htm"]
        ...
        for link in year_links:
            ...
            if yr_match:
                ...
                if "All" in self.years or yr in self.years:
                    yield response.follow(link, callback=self.parse_year, meta={"year": yr})

        # 2. Recent Calendar (2021-2026+)
        if "All" in self.years or any(int(y) >= 2021 for y in self.years if y.isdigit()):
    ...
    def parse_document(self, response):
        ...
        if is_pdf:
            ...
            title = response.css("h1::text, title::text").get(default="FOMC Document").strip()
        ...
        if not final_date or "Unknown" in final_date:
            ...
            final_date = "-".join(date_match.groups()) if date_match else datetime.now().strftime("%Y-%m-%d")
    ...
    def _classify_doc(self, url, text):
        ...
        if "minutes" in url or "minutes" in text: return "minutes"
        if "monetary" in url or "statement" in text: return "statement"
        if "implementation" in url or "note" in text: return "implementation_note"
        if "projtabl" in url or "projection" in text: return "projections"
        return "other"

    def parse_year(self, response):
        """Handle historical year pages (e.g. 2018 and older)."""
        links = response.css('a[href*="monetary"], a[href*="minutes"]::attr(href)').getall()
