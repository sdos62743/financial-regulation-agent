...
class RegulatoryCleaningPipeline:
    ...
    def process_item(self, item, spider):
        ...
        try:
            ...
            log_info(f"Successfully processed {item.get('regulator', 'UNKNOWN')} item: {item.get('title', 'No Title')[:40]}...")
            return item

        except Exception as e:
            log_error(f"Error in Pipeline for {item.get('url', 'Unknown URL')}: {str(e)}")
            return item