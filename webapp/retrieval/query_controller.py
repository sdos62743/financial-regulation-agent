...
class RAGController:
    ...
    async def ask(
        ...
        if len(query) > 2000:
            ...
            return {"error": "Query is too long (maximum 2000 characters)", "success": False}

        log_info(f"🧠 [Controller] Invoking Graph | thread_id={thread_id} | query='{query[:50]}...'")
        ...
        try:
            ...
            final_answer = (
                result.get("synthesized_response") or 
                result.get("response") or 
                "I apologize, but I couldn't generate a specific answer. Please try rephrasing."
            )
